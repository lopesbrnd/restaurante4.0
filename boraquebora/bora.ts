...
class Restaurante {
    ...
    constructor(cardapio: Prato[]) {
        ...
        this.mesas = new Array(10).fill(false);  // 10 mesas, todas inicialmente desocupadas
    }
    ...
    reservarMesa(): void {
        const numeroMesa = parseInt((document.getElementById('numeroMesa') as HTMLInputElement).value);
        const reservaMensagem = document.getElementById('reservaMensagem') as HTMLElement;

        if (isNaN(numeroMesa) || numeroMesa < 1 || numeroMesa > 10) {
            reservaMensagem.textContent = "Por favor, escolha um número de mesa entre 1 e 10.";
            reservaMensagem.classList.remove('mesa-disponivel');
            reservaMensagem.classList.add('mesa-ocupada');
            return;
        }

        if (this.mesas[numeroMesa - 1]) {
            reservaMensagem.textContent = `Mesa ${numeroMesa} já está ocupada!`;
            reservaMensagem.classList.remove('mesa-disponivel');
            reservaMensagem.classList.add('mesa-ocupada');
        } else {
            this.mesas[numeroMesa - 1] = true;
            reservaMensagem.textContent = `Mesa ${numeroMesa} reservada com sucesso!`;
            reservaMensagem.classList.remove('mesa-ocupada');
            reservaMensagem.classList.add('mesa-disponivel');
        }
    }
...
(window as any).adicionarAoPedido = function(index: number) {
